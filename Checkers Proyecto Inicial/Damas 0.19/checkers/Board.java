...
public class Board 
{
    ...
    private ArrayList<Shapes.Rectangle> blocks;
    ...
    public Board(int width,int xPos,int yPos)
    {
        ...
        killedPieces= new ArrayList<Integer>();
        //kingPieces = new KingPieces[width][width];
        blocks = new ArrayList<Shapes.Rectangle>();
        ...
        if(xPos==0)
        {
            señalador1 = new Triangle(width*55,width*35,"red"); 
    ...
    {
        señalador2 = new Triangle(width*15,width*35,"red");
    ...
    private void drawBoard(int width)
    {
       ...
       {        
           ...
           {             
              if(cont%2==0)
              {
                   Shapes.Rectangle rectangle = new Shapes.Rectangle(10+(i*60)+xPos,10+(j*30),"black");
                   blocks.add(rectangle);
                   Shapes.Rectangle blank = new Shapes.Rectangle(10+(i*60)+30+xPos,10+(j*30),"gray");
                   blocks.add(blank);
              } 
              else if(cont%2!=0)
              {
                   Shapes.Rectangle rectangle = new Shapes.Rectangle(10+(i*60)+30+xPos,10+(j*30),"black");
                   blocks.add(rectangle);
                   Shapes.Rectangle blank = new Shapes.Rectangle(10+(i*60)+xPos,10+(j*30),"gray");
                   blocks.add(blank);
              }
              cont++;
           }
    ...
    public boolean addKing(int row,int column,String color)
    {
        ...
        if(sum%2!=0)
        {
            Piece king = new KingPieces(row-1,column-1,color,xPos);
            pieces[row-1][column-1]=king;
    ...
    public boolean add(int row,int column,String color)
    {
        ...
        if(sum%2!=0)
        {
            Piece piece= new Pieces(row-1,column-1,color,xPos);
...
-b-.-.-..-.-.-.--.-.-.-.B-.-w-.--.-.-W-.w-.-.-.--.-w-w-..-.-.-.-
    ...
    {   
        ...
        Piece pieceObject=pieces[moveOp.get(notationArray[0])[0]-1][moveOp.get(notationArray[0])[1]-1];
        String newColor=pieceObject.getColor();
        pieceObject.select();
        if(pieceObject instanceof Proletarian){
                //System.out.println("entro Proletarian");
                remove(moveOp.get(notationArray[0])[0],moveOp.get(notationArray[0])[1]);
        }else{
            for(int j=1;j<notationArray.length;j++)
            {
                remove(moveOp.get(notationArray[j-1])[0],moveOp.get(notationArray[j-1])[1]);            
                if(moveOp.get(notationArray[j])[0]==1 || moveOp.get(notationArray[j])[0]==8)
                {
                    addKing(moveOp.get(notationArray[j])[0],moveOp.get(notationArray[j])[1],newColor);
                }else
                {
                    add(moveOp.get(notationArray[j])[0],moveOp.get(notationArray[j])[1],newColor);
                }
            }
            if(pieceObject instanceof Hurried)
            {
                select(moveOp.get(notationArray[notationArray.length-1])[0],moveOp.get(notationArray[notationArray.length-1])[1]);
                repetir(notationArray);
            }
        }       
    }
    private void repetir(String[] notationArray)
    {
        String posInicial=notationArray[notationArray.length-2];
        String posFinal=notationArray[notationArray.length-1];
        int direction=Integer.parseInt(posFinal)-Integer.parseInt(posInicial);
        if(direction>0){
            shift(false,true);
        }
        else if (direction<0){
    ...
    {   
        ...
        for(int j=1;j<notationArray.length;j++)
        {
            select(moveOp.get(notationArray[0])[0],moveOp.get(notationArray[0])[1]);
            int x=0;
            x=Integer.parseInt(notationArray[0])-Integer.parseInt(notationArray[1]);
            switch(x)
            {
                case 7:
                    jump(true,true);
     ...
     * @param column entero, posicion en columna
     */
    /*public void kingSelect(int row,int column,int times)
    {        
      if(kingPieces[row-1][column-1]!=null)
      {
            isSelect=true;
            kingPieces[row-1][column-1].select(times);
    ...
    {
        //System.out.println("entro mover");
        if(pieceSelect!=null)
        {
                if (notation=="left")
                {
                    if((pieceSelectpos[0]+(1*salta))<width && (pieceSelectpos[1]-(1*salta))>0)
                    {
                        ...
                        BoardInfo[pieceSelectpos[0]+(1*salta)][pieceSelectpos[1]-(1*salta)]=BoardInfo[pieceSelectpos[0]][pieceSelectpos[1]];
    ...
    {//{{5,4},{4,5}}
        if(top && right)
        {
                if(comprobar(pieceSelectpos[0]-1, pieceSelectpos[1]+1) && comprobar(pieceSelectpos[0]-2, pieceSelectpos[1]+2) && isFill[pieceSelectpos[0]-1][pieceSelectpos[1]+1]==true && isFill[pieceSelectpos[0]-2][pieceSelectpos[1]+2]==false)
                {               
                    //moveKingTop("right",2);
                    moveTop("right",2);
                    kill(pieceSelectpos[0]-1,pieceSelectpos[1]+1,pieceSelectpos[0]-2,pieceSelectpos[1]+2);
    ...
    {//{{8,1},{7,2},{7,4},{7,6}}
        if(comprobar(row-1,column-1) && !(pieces[pieceSelectpos[0]][pieceSelectpos[1]] instanceof Lazy) && !(pieces[pieceSelectpos[0]][pieceSelectpos[1]] instanceof Priest) && !(pieces[pieceSelectpos[0]][pieceSelectpos[1]] instanceof Libertarian))
        {
    ...
    public int move(boolean top, boolean right)
    {   
        MovementNum=0;
        jump(top,right);
        //System.out.println(MovementNum);
        return MovementNum;
    }
    ...
    public boolean addType(String type, int row, int column, String color)
    {
        Piece piece;
        int sum=row+column;
        if(sum%2!=0)
        {   
            if(type=="Powerful"){
                piece= new Powerful(row-1,column-1,color,xPos);
                if(color=="white")
                {
                    BoardInfo[row-1][column-1]='o';
                }else if(color=="blue"){
                    BoardInfo[row-1][column-1]='f';
                }
            }else if(type=="Libertarian"){
                piece= new Libertarian(row-1,column-1,color,xPos);
                if(color=="white")
                {
                    BoardInfo[row-1][column-1]='i';
                }else if(color=="blue"){
                    BoardInfo[row-1][column-1]='e';
                }
            }else if(type=="Lazy"){
                piece= new Lazy(row-1,column-1,color,xPos);
                if(color=="white")
                {
                    BoardInfo[row-1][column-1]='l';
                }else if(color=="blue"){
                    BoardInfo[row-1][column-1]='a';
                }
            }else if(type=="Proletarian"){
                piece= new Proletarian(row-1,column-1,color,xPos);    
                if(color=="white")
                {
                    BoardInfo[row-1][column-1]='p';
                }else if(color=="blue"){
                    BoardInfo[row-1][column-1]='r';
                }
            }else if(type=="Hurried"){
                piece= new Hurried(row-1,column-1,color,xPos);                
                if(color=="white")
                {
                    BoardInfo[row-1][column-1]='h';
                }else if(color=="blue"){
                    BoardInfo[row-1][column-1]='u';
                }
            }else if(type=="Priest"){
                piece= new Priest(row-1,column-1,color,xPos);                
                if(color=="white")
                {
                    BoardInfo[row-1][column-1]='s';
                }else if(color=="blue"){
                    BoardInfo[row-1][column-1]='t';
                }
            }else{
                piece= new Pieces(row-1,column-1,color,xPos);
                if(color=="white")
                {
                    BoardInfo[row-1][column-1]='w';
                }else if(color=="blue"){
     ...
     * @param column entero, columna coordenada y
     */
    private void kill(int row, int column,int rowKiller, int columnKiller)
    {
        //System.out.println("Entro a matar");
        if(!(pieces[row][column] instanceof Powerful) && !(pieces[rowKiller][columnKiller] instanceof Libertarian))
        {
            remove(row+1,column+1);
            //System.out.println(rowKiller);
            //System.out.println(columnKiller);
            //System.out.println(pieces[rowKiller][columnKiller] instanceof Priest);
            if(pieces[rowKiller][columnKiller] instanceof Priest)
            {
                String wololo=pieces[rowKiller][columnKiller].getColor();
                add(row+1,column+1,wololo);
            }
        }
    }
    ...
    public void readBoard(char[][] input)
    {
        ...
        for(int i=0; i <input.length;i++) 
        {
            for(int j=0; j <input.length;j++) 
            {
                ...
                {
                    //Cambiar el color
                    addType("Powerful", i+1, j+1, "white");
                }
                else if(input[i][j] == 'f')
                {
                    //Cambiar el color
                    addType("Powerful", i+1, j+1, "blue");
                }
                else if(input[i][j] == 'i')
                {
                    //Cambiar el color
                    addType("Libertarian", i+1, j+1, "white");
                }
                else if(input[i][j] == 'e')
                {
                    //Cambiar el color
                    addType("Libertarian", i+1, j+1, "blue");
                }
                else if(input[i][j] == 'l')
                {
                    //Cambiar el color
                    addType("Lazy", i+1, j+1, "white");
                }
                else if(input[i][j] == 'a')
                {
                    //Cambiar el color
                    addType("Lazy", i+1, j+1, "blue");
                }
                else if(input[i][j] == 'p')
                {
                    //Cambiar el color
                    addType("Proletarian", i+1, j+1, "white");
                }
                else if(input[i][j] == 'r')
                {
                    //Cambiar el color
                    addType("Proletarian", i+1, j+1, "blue");
                }
                else if(input[i][j] == 'h')
                {
                    //Cambiar el color
                    addType("Hurried", i+1, j+1, "white");
                }
                else if(input[i][j] == 'u')
                {
                    //Cambiar el color
                    addType("Hurried", i+1, j+1, "blue");
                }
                else if(input[i][j] == 's')
                {
                    //Cambiar el color
                    addType("Priest", i+1, j+1, "white");
                }
                else if(input[i][j] == 't')
                {
                    //Cambiar el color
                    addType("Priest", i+1, j+1, "blue");
                }
            }
        }
        BoardInfo=input;
