...
* @version 0.18 (02/03/2020)
* @version 0.19 (06/03/2020)
*/
public class Checkers   
{
        ...
        public void move()//Se sale de rango al verificar las zonas
        {
            String Simulation=write();
            select(rowSelect,columnSelect);
            int RightTop=0;
            int RightDown=0;
            int LeftTop=0;
            int LeftDown=0;
            int[] movements = new int[4];
            if(comprobar(rowSelect-1, columnSelect+1))
            {
                RightTop=boardActive.move(true,true);
            }
            movements[0]=RightTop;
            read(Simulation);
            select(rowSelect,columnSelect);
            if(comprobar(rowSelect+1, columnSelect+1))
            {
                RightDown=boardActive.move(false,true);
            }
            movements[1]=RightDown;
            read(Simulation);
            select(rowSelect,columnSelect);
            if(comprobar(rowSelect-1, columnSelect-1))
            {
                LeftTop=boardActive.move(true,false);
            }
            movements[2]=LeftTop;
            read(Simulation);
            select(rowSelect,columnSelect);
            if(comprobar(rowSelect+1, columnSelect-1))
            {
                LeftDown=boardActive.move(false,false);
            }
            movements[3]=LeftDown;
            String max=bestMax(movements);
            read(Simulation);
            bestMovement(max);
            //convertirARey();
        }
        
        private String bestMax(int[] movements)
        {
            String max="";
            int maxi=0;
            for(int i=0;i<movements.length;i++)
            {
               maxi = (movements[i]>maxi)?movements[i]:maxi;
            }
            if(maxi==movements[0]){
                max="RightTop";
            }else if(maxi==movements[1]){
                max="RightDown";
            }else if(maxi==movements[2]){
                max="LeftTop";
            }else if(maxi==movements[3]){
                max="LeftDown";
            }         
            return max;
        }
        ...
        private void bestMovement(String max)
        {
            swap();
            swap();
            select(rowSelect,columnSelect);
            ...
            {
                jump(true,true);
            }else if(max=="RightDown")
            {
                jump(false,true);
            }else if(max=="LeftTop")
            {
                jump(true,false);
            }else if(max=="LeftDown")
            { 
                jump(false,false);
            }
        }
        ...
        public void select(int row,int column)
        {
            ...
            //boardActive.kingSelect(row,column,times);
            itWorks=true;
        } 
        ...
        public void add(String type, boolean king, int row, int column, String color)
        {
            if(row<=width && column <=width){
                if(!isSwapped){
                    if(!boardActive.isFill[row-1][column-1])
                    {
                        if(king)
                        {
                            if(!boardActive.addKing(row,column,color))
                            {
                        ...
                        {
                            if(!boardActive.addType(type,row,column,color))
                            {
                               JOptionPane.showMessageDialog(null,"Casilla invalida #JEG");
                            } else
                            {
        ...
        public void add(String type, int[][] men, String color)
        {
            if(!isSwapped){
                for (int i=0; i<men.length; i++)
                {
                    int row=men[i][0];
                    int column=men[i][1];
                    add(type,false,row,column,color);
                }
                itWorks=true;
            }else
            {
         ...
         * @param column entero, columna coordenada y
         */
        public void add(boolean king ,int row,int column,String color)
        {
            if(row<=width && column <=width){
                if(!isSwapped){
                    if(!boardActive.isFill[row-1][column-1])
                    {
                        ...
                        {
                            if(!boardActive.add(row,column,color))
                            {
                               JOptionPane.showMessageDialog(null,"Casilla invalida #JEG");
                            } else
                            {
         ...
         */
        public void add(int[][] men, String color)
        {
            if(!isSwapped){
                for (int i=0; i<men.length; i++)
                {
